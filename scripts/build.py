...
def url(item):
    return "https://twitter.com/drmowinckels/status/" + item["id_str"]
...
<head>
  ...
  <title>Dr. Mowinckel's's twitter archive</title>
...
<body>
  <div class="wrapper" id="app">
    <h1><a href="../../..">Dr. Mowinckel's's twitter archive</a></h1>
...
def main():
    data = read_data()
    for item in data:
        os.makedirs(f"drmowinckels/status/{item['id_str']}", exist_ok=True)
        with open(f"./drmowinckels/status/{item['id_str']}/index.html", "w") as f:
